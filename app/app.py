...
if "user_map" not in st.session_state: # Mapa para associar telefone ao nome na visão conjunta
    st.session_state.user_map = {}

# Estado do dropdown de visualização
if "view_mode" not in st.session_state:
    st.session_state.view_mode = "personal" # 'personal' ou 'joint'

# --- Lógica de Autenticação e Senha (sem alterações) ---
...
# FUNÇÃO CENTRAL MODIFICADA: Orquestra a busca de dados com base no modo de visualização
def fetch_spending_data():
    with st.spinner("Buscando dados..."):
        ...
        if st.session_state.view_mode == "joint" and st.session_state.clientData.get('spendings_sharing_key'):
            ...
            # Garante que o nome do próprio usuário esteja no mapa para consistência
            st.session_state.user_map = {st.session_state.clientData['phone_number']: st.session_state.clientData['full_name']}

        # Busca os gastos se houver números para buscar
...
def getDataFrameBarChart():
    ...
    # Adiciona o nome do membro ao DataFrame
    dataFrame['Membro'] = dataFrame['client_phone_number'].map(st.session_state.user_map).fillna("Desconhecido")

    # Agrupa por categoria e membro
    spendingsByCategory = dataFrame.groupby(['spending_category', 'Membro'])['spending_value'].sum().reset_index()
    ...
    # Adiciona lógica de cores para a visualização pessoal
    ...
    if 'outros' in category_colors: # Garante que 'outros' tenha uma cor consistente se existir
...
def getDataFramePieChart():
    ...
    # Para o gráfico de pizza, somamos tudo, ignorando o membro individual
    dataFrame = pd.DataFrame(st.session_state.clientSpendings)
    dataFrame['spending_value'] = pd.to_numeric(dataFrame['spending_value'])
    dataFrame = dataFrame[dataFrame['spending_value'] >= 0].copy()
    ...
    # Lógica de cores (pode ser movida para uma função separada se desejado)
    colors = ['#FF4B4B','#4B7BFF','#32CD32', '#FFA500','#A34BFF','#00CED1', '#FF69B4', '#FFD700','#A0522D','#708090']
    category_colors = {cat: colors[i % len(colors)] for i, cat in enumerate(sorted(spendingsByCategory['Categoria'].unique()))}
    if 'outros' in category_colors: # Garante que 'outros' tenha uma cor consistente se existir
        category_colors['outros'] = '#708090'
...
def getLineChartDataFrame():
    ...
    # O gráfico de linha mostrará o total do grupo, sem dividir por membro
...
# --- Funções de Validação e Autenticação (sem alterações) ---
    ...
    if not email or not password or not fullName or not cpf:
        ...
        if validEmail and validPassword and validWhatsapp and validCPF and not whatsappInUse:
            try:
                ...
                user = response.user
                if user:
                    st.session_state.supabaseClient.updateClientData(phoneNumber=whatsapp,fullName=fullName,cpf=cpf,authUserId=user.id,active_subscription=False, spendingsSharingKey=None)
    ...
    if not email or not password:
        ...
        if validEmail and validPassword:
            try:
                ...
                if 'code' in dir(e) and e.code == 'invalid_credentials':
                    st.error('Credenciais inválidas! Verifique o email e a senha e tente novamente.')
                elif 'code' in dir(e) and e.code == 'email_not_confirmed':
...
def userDashboard():
    ...
    if st.session_state.spendingDataFetched == False:
        ...
        if st.session_state.clientData['active_subscription'] == False:
            ...
            dates_input_cols = st.columns([1, 1, 1.5])
            ...
            if st.session_state.clientData.get('spendings_sharing_key'):
                with dates_input_cols[2]:
                    st.selectbox(
                        ...
                        options=[
                            "Visualizar apenas meus gastos",
                            f"Visualizar gastos em conjunto"
                        ],
            ...
            if not st.session_state.clientSpendings:
                ...
                with barChartTab:
                    # --- LÓGICA CONDICIONAL PARA O GRÁFICO DE BARRAS ---
                    df_bar = getDataFrameBarChart()
                    
                    if st.session_state.view_mode == 'personal':
                        # Agrega os dados para a visão pessoal, pois o DF vem agrupado por membro
                        df_personal = df_bar.groupby(['Categoria', 'colors'], as_index=False)['Valor Gasto (R$)'].sum()
                        
                        barChart = alt.Chart(df_personal).mark_bar(
                            ...
                            color=alt.Color('Categoria:N', 
                                            scale=alt.Scale(domain=df_personal['Categoria'].tolist(), range=df_personal['colors'].tolist()),
                                            legend=None), # Esconde a legenda na visão pessoal
                            tooltip=['Categoria', 'Valor Gasto (R$)']
                        )
                    else: # MODO 'JOINT'
                        barChart = alt.Chart(df_bar).mark_bar(
                            cornerRadiusTopLeft=3,
                            cornerRadiusTopRight=3
                        ).encode(
                            x=alt.X('Categoria:N', title='Categoria', sort=None),
                            y=alt.Y('Valor Gasto (R$):Q', title='Valor Gasto (R$)', scale=alt.Scale(zero=True)),
                            color=alt.Color('Membro:N', title='Membro'), # Cor por membro
                ...
                with pieChartTab:
                    ...
                    pie_chart = alt.Chart(df_pie).mark_arc(
                        innerRadius=90,
                        cornerRadius=5
                    ).encode(
                        ...
                        color=alt.Color('Categoria:N',
                            scale=alt.Scale(
                                domain=df_pie['Categoria'].tolist(),
                                range=df_pie['colors'].tolist()
                            ),
                            legend=alt.Legend(title="Categorias", orient="right")
                        ),
                        tooltip=['Categoria', 'Valor Gasto (R$)']
                    ).configure_view(
                        strokeWidth=0
...
# --- Página de Edição e Funções de Validação (sem alterações na lógica principal) ---
...
def isSpendingsSharingKeyValid(spendingsSharingKey):
    if spendingsSharingKey is None:
        return True
    keyToAnalyse = spendingsSharingKey.strip()
    emptyKey = keyToAnalyse == ""
...
def doUpdatePersonalData(fullName:str,whatsappNumber:str,cpf:str,spendingsSharingKey:str):
    ...
    if not validSpendingsSharingKey:
        st.toast('Chave de compartilhamento deve ter 8 caracteres ou mais e não deve conter espaços')
...
def editPersonalDataPage():
    ...
    spendingsSharingKey = st.text_input(
        ...
        help="Crie uma chave de no mínimo 8 caracteres, sem espaços, e compartilhe com quem você deseja visualizar os gastos em conjunto. Para sair de um grupo, deixe este campo em branco."
    )
...
# --- Dashboard de Gerenciamento (Admin) ---
...
def managementDashboard():
    ...
    with st.container(border=True):
        ...
        with col3:
            st.write("Telefone")
        with col4:
            st.write("Assinatura Ativa")

    for clientData in st.session_state.allClientsData:
        with st.container(border=True):
            name,cpf,phoneNumber,activeSubscription = st.columns(4)
            with name:
                st.write(clientData.get('full_name'))
            with cpf:
                st.write(clientData.get('cpf'))
            with phoneNumber:
                st.write(clientData.get('phone_number'))
            with activeSubscription:
                st.toggle(
                    label='',
                    key=f'toggle_active_subscription{clientData["id"]}',
                    value=clientData.get('active_subscription', False),
                    on_change=toggleSubscriptionStatus,
                    kwargs={
                        'clientId':clientData['id'],
                        'newStatus':not clientData.get('active_subscription', False),
                        'clientName':clientData.get('full_name'),
                    }
                )
